...
$('.marquee').marquee({
	...
	gap: 50, // 滚动一个周期的间距（单位：px），默认为20
	//time in milliseconds before the marquee will start animating
	delayBeforeStart: 0, //滚动开始延迟时间（单位：毫秒），默认为1000
	//'left' or 'right'
	direction: 'left',
	//true or false - should the marquee be duplicated to show an effect of continues flow
	duplicated: true //	无缝衔接，默认为false
	 // css3easing 	// css3过渡效果，默认为linear。包含linear、swing等在内一共32种，其本质为贝塞尔曲线（三次）
});
```
