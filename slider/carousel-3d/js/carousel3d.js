(function (c) {
    function ga(z, s) {
        ...
        function ha(b) {
            ...
            Z.push(d);
            d.css({width: a.settings.itemWidth, h: a.settings.itemHeight}).data({w: a.settings.itemWidth, h: a.settings.itemHeight, index: b}).addClass("out").bind({mouseover: function () {
                ...
                var e = {type: "itemMouseOver", index: b, data: j[b]};
        ...
        function N(b) {
            var d = j[b].path, e = Z[b];
            c("<img/>").load(function () {
                var h = parseInt(c(this).attr("width") || c(this).prop("width")), f = parseInt(c(this).attr("height") || c(this).prop("height"));
                if (a.settings.crop)e.css("background-image", "url(" + d + ")"); else {
                    e.attr("src", d);
        ...
        function o(b) {
            i.find(".carousel-item").each(function (d) {
                var e = c(this), h = e.data("w"), f = e.data("h"), D = Math.sin(-(p * d) + v * 0.5 + b * w) * a.settings.verticalRadius + P - f * 0.5, r = (D - ($ - f * 0.5)) / (aa - $) * (1 - a.settings.scaleRatio) + a.settings.scaleRatio;
                e.css({width: h *
        ...
        function A() {
            if (!(G && a.settings.pauseAutoScrollIfTooltip)) {
                ...
                t = setTimeout(function () {
                    if (a.settings.autoScrollDirection == "next")ba(); else a.settings.autoScrollDirection == "previous" && ca()
        ...
        function O() {
            ...
            var b = 0, d = a.settings.mouseScrollReverse ? -1 : 1;
        ...
        function X() {
            ...
            var e = 0, h = a.settings.mouseDragReverse ? 1 :
                -1;
        ...
        function Y() {
            ...
            var b = 0, d = a.settings.mouseWheelReverse ? -1 : 1;
        ...
        function ia() {
            ...
            function d() {
                if (n < 0)n = 0; else if (n > parseInt(h.css("width")) - parseInt(f.css("width")))n = parseInt(h.css("width")) - parseInt(f.css("width"));
                x && f.css("left", n);
            ...
            var e =
                c('<div class="scrollbar"></div>').appendTo(i), h = c('<div class="track"></div>').appendTo(e), f = c('<div class="thumb"></div>').appendTo(h), D = c('<div class="left"></div>').appendTo(e), r = c('<div class="right"></div>').appendTo(e), n = 0, ea;
        ...
        function la(b) {
            var d = i.find(".scrollbar").find(".track"), e = d.find(".thumb");
        ...
        function ja(b) {
            if (b = j[b].tooltip) {
                ...
                var e = -d.outerWidth() / 2, h = 0 - d.outerHeight() - parseInt(d.css("marginBottom"));
                d.css({left: k - i.offset().left + e, top: q - i.offset().top +
                    h});
                c(document).bind("mousemove.tooltip", function () {
                    d.css({left: k - i.offset().left + e, top: q - i.offset().top + h})
                });
        ...
        function ka() {
            if (G) {
                ...
                b.stop().animate({opacity: 0}, 200, function () {
        ...
        this.settings = c.extend({}, c.fn.carousel3d.defaults, s);
        var i = c(z), a = this, l = 0, j = [], Z = [], v = Math.PI, w = v / 180, S, t, H, J,
            L, V, B = a.settings.mouseScrollSpeed, k, q, T = 0, da = 0, I = false, K = false, x = false, M = false, U = 0, g = 0, Q = a.settings.width / 2, P = a.settings.height / 2, $ = P - a.settings.verticalRadius, aa = P + a.settings.verticalRadius, p, u, y = false, G = false;
        (function () {
            i.addClass("carousel").css({width: a.settings.width, height: a.settings.height});
            if (a.settings.xmlSource) {
                i.empty();
                c.ajax({type: "GET", url: a.settings.xmlSource, dataType: c.browser.msie ? "text" : "xml", success: function (b) {
                    var d;
                    if (c.browser.msie) {
    ...
    c.fn.carousel3d.defaults = {xmlSource: null, width: 500, height: 300, itemWidth: 100, itemHeight: 100, horizontalRadius: 250, verticalRadius: 100, resize: true, maintainAspectRatio: true, crop: false, scaleRatio: 0.5, mouseScroll: false, scrollOnClick: true, mouseDrag: false, scrollbar: false, arrowScrollAmount: 50,
