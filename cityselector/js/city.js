!(function( $ ){
    ...
    citySelector.template = '<div class="city-box" id="js_cityBox"><div><button class="btn btn-back" type="button">返回</button></div><div class="prov-city" id="js_provCity"><p>热门城市</p><ul>' + citySelector.hotCityhtmls + '</ul></div><div class="provence"><div>选择省份</div><div><ul id="js_provList">' + citySelector.provHtmls + '</ul></div></div></div>';

    citySelector.cityInit = function ( selector ) {

        $(selector).click(function () {

            $("#js_cityBox").remove();
            $("body").append($(citySelector.template).fadeIn());
            ...
            $("#js_provList").off('click.cityselector').on('click.cityselector', ".provinceName", function (e) {
                function createUl(_this){
                    ...
                    cityHtmls += "</ul>";
                    $("#js_provCitys").remove();
                    _this.parent("li").append($(cityHtmls).fadeIn());
                }

                if (label == "false") {
                    label = "true";
                    $(this).attr("now-item", "true").addClass('active');
                    province = $(this).html();
                    createUl($(this));
                }
                else {
                    if ($(this).attr("now-item") == "" || $(this).attr("now-item") == "false" || $(this).attr("now-item") == undefined) {
                        $(this).parents("#js_provList").find("span").attr("now-item", "false").removeClass('active')
                        $(this).attr("now-item", "true").addClass('active')
                        $("#js_provCitys").remove();
                        province = $(this).html();
                        createUl($(this));
            ...
            $("#js_cityBox").off('click.cityselector').on("click", ".js_cityName", function (e) {
                e.stopPropagation();
                province = province ? province + ',' : province;
                $(selector).val( province + $(this).html());
                $("#js_cityBox").remove();
            }).on("click", function (e) {
                if( !$(e.target).hasClass('provinceName') ){
                    $("#js_provCitys").remove();
                }
